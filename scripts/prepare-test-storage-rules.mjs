...
const TEMPLATE_PATH = path.join(rootDir, 'storage.rules.template');
const OUT_DIR = path.join(rootDir, '.generated');
const OUT_PATH = path.join(OUT_DIR, 'storage.test.rules');
...
if (!fs.existsSync(TEMPLATE_PATH)) {
  console.error(`ERROR: Canonical Storage template not found at ${TEMPLATE_PATH}`);
  process.exit(1);
}
...
console.log(`Successfully prepared test Storage rules with test UID: ${TEST_OWNER_UID}`);
